...
import com.fifththirdbankcc.databinding.FragmentDailyBlondeJokeBinding
...
class DailyBlondeJokeFragment : BaseFragment() {

    private val binding by lazy {
        FragmentDailyBlondeJokeBinding.inflate(layoutInflater)
    }
        ...
        jokeViewModel.joke.observe(viewLifecycleOwner) { state ->
            when (state) {
                ...
                is JokeResult.ERROR -> {
                    ...
                    showErrorDialog(state.error, FragmentName.BLONDE) {
                        jokeViewModel.subscribeToDailyJoke(JokeCategory.BLONDE)
                    }
                }
            }
        }
